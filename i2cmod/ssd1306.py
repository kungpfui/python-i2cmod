...
class SSD1306(SMBus):
	"""
	"""
	def __init__(self, bus=1, device_addr=0x3c, font_size=16, contrast=0x7F):
		"""
	...
	def set_font(self, font="/root/sensors/fonts/isocpeur.ttf", font_size=16):
		self.font = ImageFont.truetype(font, font_size)
	

	def _configure(self, charge_pump=True):
		...
		self.command(Reg.memory_addr_mode, 0)               # 0 act like ks0108
	...
	def write_i2c_block_data(self, register: int, data: Union[bytes, bytearray, tuple, list]):
	...
	def update(self, buffer=None):
		...
		:param buffer:  None: buffer is filled by PIL object self.image
					   ...
					   bytearray, list, tuple:
		"""
		if buffer is None:
			return self.update_from_image()
